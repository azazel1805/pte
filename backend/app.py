...
def get_describe_image():
    ...
    # Fetch a random, interesting image from Pexels
...
def get_reorder_paragraph():
    ...
    response = gemini_model.generate_content(prompt)
    # Split the response into sentences and filter out any empty strings
...
def evaluate_spoken_response():
    ...
    task_type = data.get('taskType', 'read aloud') # e.g., 'read aloud', 'describe image'
    ...
    evaluation_prompt = f"""
    You are an expert PTE Academic examiner. Evaluate the following student's spoken response based on typical PTE criteria: oral fluency, pronunciation, and content.
    ...
    {{
      "oral_fluency": {{
        ...
        "feedback": "<brief feedback on rhythm, phrasing, and speed>"
      }},
      "pronunciation": {{
        "score": <score out of 5>,
        "feedback": "<brief feedback on clarity, vowel/consonant sounds, and stress>"
      }},
      "content": {{
        "score": <score out of 5>,
        "feedback": "<brief feedback on how well the transcript matches the original text or describes the key elements of the task>"
      }},
